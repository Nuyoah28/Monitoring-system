<template>
  <view class="main owner-app" :style="{ minHeight: safeHeight + 'px', height: safeHeight + 'px', paddingTop: statusBarHeight + 'px', '--status-bar-height': statusBarHeight + 'px' }">
    <view class="header">
      <view class="top-nav">
      ...
      <text class="header-title">社区助手</text>
    </view>
    ...
    <view class="body" id="ai-body" :class="{ 'chat-open': showChatPanel }" :style="bodyStyle">
      ...
      <view class="assistant-dock">
        <view class="assistant-dock-title">社区助手</view>
        <view class="assistant-chip-row">
          <view class="assistant-chip">提醒</view>
          <view class="assistant-chip">报修</view>
...
const AI_WELCOME_MESSAGE = '你好，我是社区助手，面向业主为你提供社区提醒、报修进度、环境数据、停车服务和访客登记说明。有什么需要我帮你查看或解释的，直接问我就好。';
...
export default {
  ...
  computed: {
    ...
    sessionCacheKey() {
      const appType = uni.getStorageSync('appType') || 'owner';
  ...
  methods: {
      ...
      try {
        ...
        if (parsed && Array.isArray(parsed.sessions)) {
          this.sessions = parsed.sessions.map((session) => ({
            ...session,
            messages: this.normalizeSessionMessages(session.messages),
          }));
    ...
    switchSession(id, closePanel = true) {
      ...
      this.textList = this.normalizeSessionMessages(messages);
      target.messages = [...this.textList];
    ...
    normalizeSessionMessages(messages) {
      ...
      const normalized = list.map((item) => (
        item && typeof item === 'object' && item.type === 'welcome'
          ? this.createWelcomeMessage()
          : item
      ));
      const hasWelcome = normalized.some((item) => item && typeof item === 'object' && item.type === 'welcome');
      if (!hasWelcome) normalized.unshift(this.createWelcomeMessage());
      return normalized.length ? normalized : [this.createWelcomeMessage()];
    },
    getMessageText(item) {
      if (item == null) return '';
...
.assistant-dock {
  ...
  border: 1px solid rgba(149, 217, 255, 0.18);
  box-shadow: 0 12rpx 32rpx rgba(0, 12, 40, 0.18);
  box-sizing: border-box;
