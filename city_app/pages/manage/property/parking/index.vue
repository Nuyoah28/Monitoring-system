  ...
  <view class="feature-page">
    ...
    <view class="panel form-panel">
      ...
      <view class="form-row">
        ...
        <view class="form-item half">
          ...
          <input v-model="form.occupiedVehicle" class="ipt" placeholder="如：35" />
        </view>
      </view>
    ...
    <view class="panel list-panel">
      ...
      <view v-for="item in spaces" :key="item.id" class="record-card">
        ...
        <view class="meta">占用车辆数：{{ parseOccupiedCount(item.occupiedVehicle) }}</view>
      </view>
    </view>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    async submitSpace() {
      ...
      if (Number.isNaN(occupiedCount) || occupiedCount < 0) {
