...
- `alarm_cnt`
  累计告警次数或当前统计的告警数量
- `stream_link`
  视频流地址，例如 RTMP、RTSP 或其他流媒体地址
- `running`
  该监控点是否启用或在线，`0` 表示关闭/未运行，`1` 表示运行中
- `danger_area`
  是否启用危险区域识别
- `fall`
  是否启用摔倒识别
...
- `wave`
  是否启用挥手呼救识别
- `left_x`
  监控画面中某个业务区域左上角或左边界的 X 坐标
- `left_y`
  监控画面中某个业务区域左上角或左边界的 Y 坐标
- `right_x`
  监控画面中某个业务区域右下角或右边界的 X 坐标
- `right_y`
  监控画面中某个业务区域右下角或右边界的 Y 坐标
- `latitude`
  监控点纬度
- `longitude`
  监控点经度
...
- 一组 `left_x / left_y / right_x / right_y` 通常用来表达算法关注区域
...
- `clip_link`
  告警视频片段链接、文件名或素材标识
- `monitor_id`
  触发告警的监控点 ID，对应 `monitor.id`
- `case_type`
  告警类型 ID，对应 `case_type_info.id`
- `warning_level`
...
- `push_status`
  ...
  `success`、`fail`
- `push_detail`
  推送结果补充说明，例如错误信息、失败原因、返回内容摘要
- `created_time`
  推送记录创建时间

说明：

- 同一个 `alarm_id + user_id + push_type` 在表中通常只保留一条唯一记录
...
- 这张表更像“基础停车信息表”
- 更完整的动态停车统计主要在 `parking_area_status` 和 `parking_area_record`
...
- 一般同一个 `monitor_id + area_code` 只保留一条当前状态
