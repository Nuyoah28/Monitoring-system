...
class FlowAggregator:
    def __init__(self) -> None:
        self.total_counts: dict[str, dict[str, dict[str, int]]] = defaultdict(
            lambda: defaultdict(lambda: defaultdict(int))
        )
        self.minute_buckets: dict[str, dict[str, dict[str, dict[str, int]]]] = defaultdict(
            lambda: defaultdict(lambda: defaultdict(lambda: defaultdict(int)))
        )

    def add_event(self, event: FlowEvent) -> None:
        self.total_counts[event.rule_id][event.direction][event.vehicle_class] += 1
        minute_key = str(event.timestamp_ms // 60000)
        self.minute_buckets[minute_key][event.rule_id][event.direction][event.vehicle_class] += 1
