...
class EUCB(nn.Module):
    ...
    def __init__(self, c1, c2=None, k=3, s=1):
        super().__init__()
        c2 = c1 if c2 is None else c2
        self.in_channels = c1
        self.up_dwc = nn.Sequential(
            nn.Upsample(scale_factor=2, mode="nearest"),
            nn.Conv2d(c1, c1, kernel_size=k, stride=s, padding=k // 2, groups=c1, bias=False),
            nn.BatchNorm2d(c1),
            nn.ReLU(inplace=True),
        )
        self.pwc = nn.Conv2d(c1, c2, kernel_size=1, stride=1, padding=0, bias=True)
...
class FuseModule(nn.Module):
    ...
    def __init__(self, c_in, channel_adjust):
        ...
        # x[2] branch has 2 * c_in channels in current HyperACE design,
        # so EUCB should keep that channel width consistent with the original nn.Upsample behavior.
        self.upsample = EUCB(2 * c_in, 2 * c_in)
        if channel_adjust:
